...
BEGIN
  ...
  -- Participant check (adjust if your column names differ)
  ...
  ELSIF action_key = 'CONFIRM_HANDOVER' THEN
    ...
    -- ✅ FIXED: cast to enum handover_role
    INSERT INTO carry_request_handover_confirmations (
      carry_request_id, user_id, role, confirmed_at
    )
    VALUES (
      request_id,
      uid,
      actor_role::handover_role,
      now()
    )
    ...
    -- ✅ FIXED: compare enum values
    SELECT
      COUNT(*) FILTER (WHERE role = 'SENDER'::handover_role) > 0,
      COUNT(*) FILTER (WHERE role = 'TRAVELER'::handover_role) > 0
  ...
  -- Notification template (if your helper exists)
  SELECT * INTO tpl
