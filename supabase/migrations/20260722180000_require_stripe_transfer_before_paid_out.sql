-- Require a persisted Stripe Transfer id before RELEASE_PAYMENT can mark PAID_OUT.

alter table public.carry_requests
  add column if not exists stripe_transfer_id text;

comment on column public.carry_requests.stripe_transfer_id is
  'Stripe Transfer id for traveler payout; required before PAID_OUT when a PaymentIntent exists.';

create unique index if not exists carry_requests_stripe_transfer_id_idx
  on public.carry_requests (stripe_transfer_id)
