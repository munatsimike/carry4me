  ...
  return (
    select jsonb_build_object(
      'total_posted_parcels',
        (select count(*) from public.parcels where sender_user_id = p_user_id),

      'total_posted_trips',
        (select count(*) from public.trips where traveler_user_id = p_user_id),
      ...
      'active_requests',
        (
          ...
          where
            (sender_user_id = p_user_id or traveler_user_id = p_user_id)
            and status in (
              'PENDING_ACCEPTANCE',
              'PENDING_PAYMENT',
              'PENDING_HANDOVER',
...
begin
  ...
  select * into tpl
  from public.get_carry_request_notification_template(event_type, recipient_role, actor_role);
...
revoke execute on function public.get_dashboard_overview(uuid) from public;
revoke execute on function public.perform_carry_request_action(uuid, text) from public;
revoke execute on function public.expire_overdue_carry_requests() from public;
revoke execute on function public.trip_has_available_capacity(uuid, numeric) from public;
revoke execute on function public.get_carry_request_notification_template(text, text, text) from public;
...
grant execute on function public.trip_has_available_capacity(uuid, numeric) to authenticated;
