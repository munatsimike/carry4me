-- Expire a single overdue carry request in the DB (not just in UI).
-- Used by batch expiry, perform_carry_request_action, and edge functions.
...
begin
  ...
  loop
    if public.expire_carry_request(r.id) then
...
-- Any action on an overdue request persists EXPIRED before returning PAYMENT_EXPIRED.
...
begin
  ...
  if cr.status = 'PENDING_PAYMENT'
    ...
    perform public.expire_carry_request(request_id);
...
-- Run expiry every 5 minutes instead of once per hour.
do $$
begin
  if not exists (select 1 from pg_extension where extname = 'pg_cron') then
    raise notice 'pg_cron extension is not enabled; expire-overdue-carry-requests cron was not rescheduled.';
    return;
  end if;
...
exception
  when others then
    raise notice 'expire-overdue-carry-requests cron not rescheduled: %', sqlerrm;
end;
$$;
...
grant execute on function public.expire_overdue_carry_requests() to service_role;
