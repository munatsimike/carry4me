-- Grace period while Stripe checkout is in progress: do not expire requests
-- that already have a PaymentIntent, or after payment_status = SUCCEEDED.
...
declare
  ...
  v_checkout_grace_minutes constant integer := 30;
begin
  select
    cr.id,
    cr.trip_id,
  ...
  if coalesce(r.payment_status, '') = 'SUCCEEDED' then
    return false;
  end if;
  ...
  -- Checkout started: allow a grace window after payment_expires_at so in-flight
  -- Stripe payments can complete without the request being marked EXPIRED.
  if r.stripe_payment_intent_id is not null
     and r.payment_expires_at > now() - (v_checkout_grace_minutes || ' minutes')::interval then
    return false;
  end if;
...
-- Only return PAYMENT_EXPIRED when expire_carry_request actually expired the row.
...
begin
  ...
  if cr.status = 'PENDING_PAYMENT'
    ...
    if cr.status = 'PENDING_HANDOVER' and action_key = 'PAY' then
      return jsonb_build_object(
        'ok', true,
        'action', action_key,
  ...
  if action_key = 'CONFIRM_HANDOVER' and next_status = 'IN_TRANSIT' then
