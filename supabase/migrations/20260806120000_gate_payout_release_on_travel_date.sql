-- Gate RELEASE_PAYMENT until the trip travel date (UTC calendar day) has been reached.
...
begin
  ...
  if action_key = 'ACCEPT' then
    ...
    -- Payout only on/after the trip departure calendar day (UTC).
    -- Fail-open if departure_date is missing/unparseable so valid releases are not bricked.
    v_travel_date := null;
    begin
      if nullif(trim(cr.trip_snapshot ->> 'departure_date'), '') is not null then
        v_travel_date := (
          (cr.trip_snapshot ->> 'departure_date')::timestamptz at time zone 'utc'
        )::date;
      end if;
    exception
      when others then
    ...
    if v_travel_date is not null
       and (timezone('utc', now()))::date < v_travel_date then
      return jsonb_build_object(
        'ok', false,
    ...
    if cr.stripe_payment_intent_id is not null
       and nullif(trim(coalesce(cr.stripe_transfer_id, '')), '') is null then
      return jsonb_build_object(
        'ok', false,
        'reason', 'TRANSFER_NOT_CONFIRMED',
        'message', 'Traveler payout has not been confirmed yet. Verify the delivery code again.'
      );
    end if;
